SET SESSION FOREIGN_KEY_CHECKS=0;

/* Drop Tables */

DROP TABLE IF EXISTS LOTTERY_ORDER_DETAIL;
DROP TABLE IF EXISTS LOTTERY_ORDER;
DROP TABLE IF EXISTS TRADE_INFO;
DROP TABLE IF EXISTS ACCOUNT_DETAIL;
DROP TABLE IF EXISTS ACCOUNT_INFO;
DROP TABLE IF EXISTS LOTTERY_ITEM;
DROP TABLE IF EXISTS LOTTERY_ROUND_ITEM;
DROP TABLE IF EXISTS LOTTERY_ROUND;
DROP TABLE IF EXISTS NOTICE_INFO;
DROP TABLE IF EXISTS OFFACCOUNT_INFO;




/* Create Tables */

CREATE TABLE ACCOUNT_DETAIL
(
	ACCOUNTID INT NOT NULL AUTO_INCREMENT,
	USERID INT NOT NULL,
	USERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	LIMITED DOUBLE(18,2) DEFAULT 0.00,
	RATIO DOUBLE(18,4) DEFAULT 0.00,
	PERCENTAGE DOUBLE(18,4) DEFAULT 0.00,
	STATE CHAR DEFAULT '1',
	SUPUSERNAME VARCHAR(20) DEFAULT '',
	LEVEL VARCHAR(20) DEFAULT '',
	OFFTYPE CHAR,
	MONEY DECIMAL(19,2) DEFAULT 0.00,
	ATTRIBUTE1 VARCHAR(20),
	ATTRIBUTE2 VARCHAR(20),
	PRIMARY KEY (ACCOUNTID),
	UNIQUE (USERNAME)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;


CREATE TABLE ACCOUNT_INFO
(
	ACCOUNTID INT NOT NULL AUTO_INCREMENT,
	USERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	AUSERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	PASSWORD VARCHAR(50) DEFAULT '' NOT NULL,
	LIMITED DOUBLE(18,2) DEFAULT 0.00,
	RATIO DOUBLE(18,4) DEFAULT 0.00,
	IP VARCHAR(20) DEFAULT '',
	INPUTDATE DATETIME,
	UPDATEIP VARCHAR(20) DEFAULT '',
	UPDATEDATE DATETIME,
	STATE CHAR NOT NULL,
	SUPUSERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	LEVEL VARCHAR(20) DEFAULT '',
	PRIMARY KEY (ACCOUNTID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;


CREATE TABLE LOTTERY_ITEM
(
	-- 流水号
	ITEMID INT NOT NULL AUTO_INCREMENT COMMENT '流水号',
	-- 游戏类型：01玉米籽
	LOTTERYTYPE VARCHAR(10) NOT NULL COMMENT '游戏类型：01玉米籽',
	-- 下注项编号
	ITEMNO VARCHAR(10) NOT NULL COMMENT '下注项编号',
	-- 下注项名称
	ITEMNAME VARCHAR(50) COMMENT '下注项名称',
	-- 下注项类型：01广西快乐十分，02番摊玉米籽
	ITEMTYPE VARCHAR(10) COMMENT '下注项类型：01广西快乐十分，02番摊玉米籽',
	-- 类别，如特码、色、单双、大小等
	ITEMGROUP VARCHAR(20) COMMENT '类别，如特码、色、单双、大小等',
	-- 游戏显示的赔率
	ITEMSCALE DOUBLE(18,4) NOT NULL COMMENT '游戏显示的赔率',
	-- 赔率-1
	ITEMODDS DOUBLE(18,4) COMMENT '赔率-1',
	-- 抽水
	ITEMBONUS DOUBLE(18,4) COMMENT '抽水',
	PRIMARY KEY (ITEMID),
	UNIQUE (ITEMID)
);


CREATE TABLE LOTTERY_ORDER
(
	-- 下注单ID
	ORDERID INT NOT NULL AUTO_INCREMENT COMMENT '下注单ID',
	-- 游戏ID
	ROUNDID INT NOT NULL COMMENT '游戏ID',
	-- 帐户ID
	ACCOUNTID INT NOT NULL COMMENT '帐户ID',
	-- 下注金额
	ORDERAMOUNT DOUBLE(18,2) COMMENT '下注金额',
	-- 代理提成
	COMMISIONAMOUNT DOUBLE(18,2) COMMENT '代理提成',
	ORDERTIME DATETIME NOT NULL,
	-- 中奖金额
	PRIZEAMOUNT DOUBLE(18,2) COMMENT '中奖金额',
	-- 实际到手奖金
	ACTUALAMOUNT DOUBLE(18,2) COMMENT '实际到手奖金',
	-- 抽水返利
	RETURNAMOUNT DOUBLE(18,2) COMMENT '抽水返利',
	-- 中奖时间
	PRIZETIME DATETIME COMMENT '中奖时间',
	-- 帐号余额
	ACCOUNTAMOUNT DECIMAL(19,2) COMMENT '帐号余额',
	PRIMARY KEY (ORDERID),
	UNIQUE (ORDERID)
);


CREATE TABLE LOTTERY_ORDER_DETAIL
(
	-- 下注单ID
	ORDERID INT NOT NULL COMMENT '下注单ID',
	-- 下注项编号
	ITEMNO VARCHAR(10) NOT NULL COMMENT '下注项编号',
	-- 赔率-1
	ITEMODDS DOUBLE(18,4) COMMENT '赔率-1',
	-- 抽水
	ITEMBONUS DOUBLE(18,4) COMMENT '抽水',
	-- 下注金额
	DETAILAMOUNT DOUBLE(18,2) COMMENT '下注金额',
	-- 中奖金额
	PRIZEAMOUNT DOUBLE(18,2) COMMENT '中奖金额',
	-- 实际到手奖金
	ACTUALAMOUNT DOUBLE(18,2) COMMENT '实际到手奖金',
	-- 抽水返利
	RETURNAMOUNT DOUBLE(18,2) COMMENT '抽水返利',
	PRIMARY KEY (ORDERID, ITEMNO),
	UNIQUE (ORDERID, ITEMNO)
);


CREATE TABLE LOTTERY_ROUND
(
	-- 游戏ID
	ROUNDID INT NOT NULL AUTO_INCREMENT COMMENT '游戏ID',
	-- 游戏类型：01玉米籽
	LOTTERYTYPE VARCHAR(10) NOT NULL COMMENT '游戏类型：01玉米籽',
	-- 期次
	LOTTERYTERM VARCHAR(10) NOT NULL COMMENT '期次',
	-- 开出之号码
	RESULTSTR VARCHAR(100) COMMENT '开出之号码',
	-- 游戏开始时间
	STARTTIME DATETIME COMMENT '游戏开始时间',
	-- 游戏开奖时间
	ENDTIME DATETIME COMMENT '游戏开奖时间',
	-- 游戏状态：Open开盘中，Close已封盘，End已结束
	ROUNDSTATUS VARCHAR(10) NOT NULL COMMENT '游戏状态：Open开盘中，Close已封盘，End已结束',
	PRIMARY KEY (ROUNDID),
	UNIQUE (ROUNDID),
	UNIQUE (LOTTERYTYPE, LOTTERYTERM)
);


CREATE TABLE LOTTERY_ROUND_ITEM
(
	-- 游戏ID
	ROUNDID INT NOT NULL COMMENT '游戏ID',
	-- 下注项编号
	ITEMNO VARCHAR(10) NOT NULL COMMENT '下注项编号',
	-- 游戏显示的赔率
	ITEMSCALE DOUBLE(18,4) COMMENT '游戏显示的赔率',
	-- 赔率-1
	ITEMODDS DOUBLE(18,4) COMMENT '赔率-1',
	-- 抽水
	ITEMBONUS DOUBLE(18,4) COMMENT '抽水',
	-- 更新时间
	UPDATETIME DATETIME COMMENT '更新时间',
	PRIMARY KEY (ROUNDID, ITEMNO),
	UNIQUE (ROUNDID, ITEMNO)
);


CREATE TABLE NOTICE_INFO
(
	-- 公告ID
	NOTICEID INT NOT NULL AUTO_INCREMENT COMMENT '公告ID',
	-- 公告内容
	NOTICECONTENT VARCHAR(500) NOT NULL COMMENT '公告内容',
	-- 是否有效
	ISVALID BOOLEAN DEFAULT '1' NOT NULL COMMENT '是否有效',
	PRIMARY KEY (NOTICEID),
	UNIQUE (NOTICEID)
);


CREATE TABLE OFFACCOUNT_INFO
(
	ACCOUNTID INT NOT NULL AUTO_INCREMENT,
	USERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	AUSERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	PASSWORD VARCHAR(50) DEFAULT '' NOT NULL,
	LIMITED DOUBLE(18,2) DEFAULT 0.00,
	RATIO DOUBLE(18,4) DEFAULT 0.00,
	PERCENTAGE DOUBLE(18,4) DEFAULT 0.00,
	QUERY VARCHAR(50) DEFAULT '',
	MANAGE VARCHAR(50) DEFAULT '',
	IP VARCHAR(20) DEFAULT '',
	INPUTDATE DATETIME,
	UPDATEIP VARCHAR(20) DEFAULT '',
	UPDATEDATE DATETIME,
	STATE CHAR DEFAULT '1',
	SUPUSERNAME VARCHAR(20) DEFAULT '' NOT NULL,
	LEVEL VARCHAR(20) DEFAULT '',
	OFFTYPE CHAR DEFAULT '',
	ATTRIBUTE1 VARCHAR(20),
	ATTRIBUTE2 VARCHAR(20),
	PRIMARY KEY (ACCOUNTID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;


CREATE TABLE TRADE_INFO
(
	-- 流水号
	TRADEID INT NOT NULL AUTO_INCREMENT COMMENT '流水号',
	-- 帐户ID
	ACCOUNTID INT NOT NULL COMMENT '帐户ID',
	-- 业务类型：Inout出入金；Trade交易；
	TRADETYPE VARCHAR(10) NOT NULL COMMENT '业务类型：Inout出入金；Trade交易；',
	RELATIVEID INT,
	-- 业务关联类型:In入金；Out出金；Commision提成；Pay付款；Order下注；Prize奖金；Return洗码
	RELATIVETYPE VARCHAR(10) NOT NULL COMMENT '业务关联类型:In入金；Out出金；Commision提成；Pay付款；Order下注；Prize奖金；Return洗码',
	-- 交易金额
	TRADEAMOUNT DOUBLE(18,2) NOT NULL COMMENT '交易金额',
	-- 帐号余额
	ACCOUNTAMOUNT DECIMAL(19,2) COMMENT '帐号余额',
	-- 录入时间
	INPUTTIME DATETIME COMMENT '录入时间',
	REMARK VARCHAR(100),
	PRIMARY KEY (TRADEID),
	UNIQUE (TRADEID)
);



/* Create Foreign Keys */

ALTER TABLE LOTTERY_ORDER
	ADD FOREIGN KEY (ACCOUNTID)
	REFERENCES ACCOUNT_DETAIL (ACCOUNTID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE TRADE_INFO
	ADD FOREIGN KEY (ACCOUNTID)
	REFERENCES ACCOUNT_DETAIL (ACCOUNTID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE LOTTERY_ORDER_DETAIL
	ADD FOREIGN KEY (ORDERID)
	REFERENCES LOTTERY_ORDER (ORDERID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE LOTTERY_ORDER
	ADD FOREIGN KEY (ROUNDID)
	REFERENCES LOTTERY_ROUND (ROUNDID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE LOTTERY_ROUND_ITEM
	ADD FOREIGN KEY (ROUNDID)
	REFERENCES LOTTERY_ROUND (ROUNDID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;



