...
public class LotteryOrderService {
	...
	for (LotteryOrderDetail detail : order.getOrderDetailList()) {// 下注细项汇总
	...
	if (lossOrder > 0) {// 玩家输的下注项
	    addTradeInfo(account, order, order.getOrderamount(), EnumType.RalativeType.Order.ID);// 玩家输了下注本金
	    orderCommision += order.getOrderamount() * (0.8 + 0.14);// 提成
	    systemCommision += order.getOrderamount() * 0.06;
	}
